...
pum_align_order(int *order)
{
    ...
    static inline int
pum_order_item_isabbr(int *order, int index)
{
    assert(index >= 0 && index < 3);
    return order[index] == CPT_ABBR ? TRUE : FALSE;
}
...
pum_redraw(void)
{
	...
	for (j = 0; j < 3; ++j)
	{
	    ...
	    width = 0;
	    s = NULL;
	    p = pum_order_item_isabbr(order, j) ? pum_array[idx].pum_text
						: pum_get_item(idx, item_type);
	    if (p != NULL)
		for ( ; ; MB_PTR_ADV(p))
		{
		    if (s == NULL)
	    ...
	    if (j + 1 < 3)
	    {
		if (pum_order_item_isabbr(order, j+ 1))
		    next_isempty = pum_array[idx].pum_text == NULL;
		else
